...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        errors = []
        new_weights = []
        for parto in self.particles:
            self.predict_particle_odometry(parto)
            sqrd_norm = self.get_prediction_error_squared(laser_scan, parto)
            errors.append(sqrd_norm)
            new_weights.append(exp(-sqrd_norm/(self.beam_range_measurement_noise_std_dev**2)))
            #eta = eta + sqrd_norm
        n_errors = np.array(errors)
        np_new_weights = np.array(new_weights)
        zeta = np.amax(n_errors)
        #print"zeta ", zeta
        #normalized_error = 1./n_errors
        
        #print n_errors
        #print "sum of error prob: ", np.sum(n_errors)
        #self.weights = np.absolute(normalized_error - 1)
        #print "weights", self.weights
        actual_eta = np.sum(np_new_weights)
        
        self.weights = np_new_weights/actual_eta
        #print "weights", self.weights
        ...
        N_eff = len(np.unique(self.particles))
    ...
    def resample(self):
        ...
        for i in range (N):
            ...
            while beta > self.weights[index]:
                ...
                index = (index+1) % N
            x_value = self.particles[index].x + (random.uniform(-1.,1.))
            y_value = self.particles[index].y + (random.uniform(-1.,1.))
            p3.append(Particle(i, x_value, y_value, self.particles[index].theta))
            #print"appended"
        self.particles = p3
        ...
        y_values = np.array([])
        for i in range (N):
            x_values.append(self.particles[i].x)
            y_values.append(self.particles[i].y)
        cum_x_values = np.cumsum(x_values)
        cum_y_values = np.cumsum(y_values)
        for i in range (N):
        """
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        subSample = self.subsample_laser_scan(laser_scan_msg)
        simulated_ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, subSample[1], self.laser_min_range, self.laser_max_range)
        a_subSample = np.array(subSample[0])
        b_simulated_ranges = np.array(simulated_ranges)
        range_difference = (a_subSample - b_simulated_ranges)
        ...
        norm_error = np.linalg.norm(range_difference)
        return norm_error**2
