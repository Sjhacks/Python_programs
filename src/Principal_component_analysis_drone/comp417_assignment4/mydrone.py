...
def draw_objects():
    ...
    tileType = (geoclass.classifyOne(pca, clf, im2, classnames))
    print tileType
    tileTypeNumber = tileType[0]
    ...
    #Algorithm choice
    algorithm = 0
    #Algorithm 1 (Brownian motion)
    
    if(algorithm == 0):
        #print (oldp[0],oldp[1])
        if (tileTypeNumber != 2):
            xVariation = np.random.normal(0,0.5)
            yVariation = np.random.normal(0,0.5)
            tx = 5*xVariation
            ty = 5*yVariation
        else:
            tx = -tx
    ...
    #Algorithm 2 (go away from urban tiles)
    if(algorithm == 1):
        moved = 0
        if(tileTypeNumber != 2):
            while(moved == 0):
                randDirection = random.randint(0,4)
                #randDirection = 0
                print(randDirection)
                if (randDirection == 0):
                    dirLat,dirLon = ts.imagePixelsToLL( actual_pX, actual_pY, zoomLevel,  oldp[0]/(256/scalex), (oldp[1]+(256/scaley))/(256/scaley) )
                    dirIm, dirFoox, dirFooy, dirFname = ts.tiles_as_image_from_corr(dirLat, dirLon, zoomLevel, 1, 1, 0, 0)
                    dirIm2 = np.asarray(dirIm, dtype=np.float32).flatten()
                    dirTileType = (geoclass.classifyOne(pca, clf, dirIm2, classnames))
                    dirTileTypeNumber = dirTileType[0]
                    print (dirTileType)
                    if (dirTileTypeNumber !=2):
                        xVariation = np.random.normal(0,0.5)
                        yVariation = np.random.normal(0,0.5)
                        tx = 4*xVariation
                        ty = 5*abs(yVariation)
                ...
                if (randDirection == 1):
                    ...
                    if (dirTileTypeNumber !=2):
                        ...
                        continue
                if (randDirection == 2):
                    dirLat,dirLon = ts.imagePixelsToLL( actual_pX, actual_pY, zoomLevel,  (oldp[0]+(256/scalex))/(256/scalex), (oldp[1])/(256/scaley) )
                    dirIm, dirFoox, dirFooy, dirFname = ts.tiles_as_image_from_corr(dirLat, dirLon, zoomLevel, 1, 1, 0, 0)
                    dirIm2 = np.asarray(dirIm, dtype=np.float32).flatten()
                    dirTileType = (geoclass.classifyOne(pca, clf, dirIm2, classnames))
                    dirTileTypeNumber = dirTileType[0]
