...
def rrt_search(G, tx, ty):
    ...
    while found != 1:
        ...
        if pointPointDistance(x_rand, vertices[v_near]) < SMALLSTEP:
            xValue = x_rand[0]
            yValue = x_rand[1]
        else:
            theta1 = math.atan2(x_rand[1] - vertices[v_near][1], x_rand[0] - vertices[v_near][0])
            xValue = vertices[v_near][0] + SMALLSTEP*math.cos(theta1)
            yValue = vertices[v_near][1] + SMALLSTEP*math.sin(theta1)
        x_new = (xValue,yValue)
        for recto in obstacles:
            if lineHitsRect(vertices[v_near], x_new, recto):
                obstructed = 1
        if obstructed == 0:
            lengthv = len(G[nodes])
        ...
        #print("1")
        canvas.polyline([vertices[v_near], vertices[len(vertices)-1] ])
        canvas.events()
