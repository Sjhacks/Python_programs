...
start_y = 630
start_ang = 0
startPos = [start_x, start_y, start_ang]

vertices = [ [start_x,start_y, start_ang] ]
...
def genPoint():
    ...
    rand = None
    while rand is None:
        rand = (random.randint(0,XMAX),
                random.randint(0,YMAX))

        for recto in obstacles:
            if inRect(rand, recto, 1):
                rand = None
                break
    ...
    #rand = None
    #while rand is None:
        #rand = (min(XMAX, max(0,random.gauss(tx,350))),
                #min(YMAX, max(0,random.gauss(ty,350))))
...
def returnParent(k):
    ...
    for e in G[edges]:
        if e[1]==k: 
            ...
            x_parent = vertices[e[0]]
            newFrontx = x_parent[0]+ (robot_length/2)*math.cos(x_parent[2])
            newFronty = x_parent[1]+ (robot_length/2)*math.sin(x_parent[2])
            newFront = [newFrontx,newFronty]
            newBackx = x_parent[0]+ (robot_length/2)*math.cos(x_parent[2]+180)
            newBacky = x_parent[1]+ (robot_length/2)*math.sin(x_parent[2]+180)
            newBack = [newBackx,newBacky]
            canvas.polyline([newFront,newBack], style=2)
            return e[0]
...
def rrt_search(G, tx, ty):
    ...
    while found != 1:
        ...
        zValue = 0
        x_rand = genPoint()
        v_near = closestPointToPoint(G, x_rand)
        x_near = vertices[v_near]
        #if the point generated is close we can just add it to graph
        if pointPointDistance(x_rand, x_near) <= SMALLSTEP:
            xValue = x_rand[0]
            yValue = x_rand[1]
            zValue = x_rand[2]
        #otherwise we get a closer point
        else:
            theta1 = math.atan2(x_rand[1] - x_near[1], x_rand[0] - x_near[0])
            xValue = x_near[0] + SMALLSTEP*math.cos(theta1)
            yValue = x_near[1] + SMALLSTEP*math.sin(theta1)
            zValue = x_rand[2]
        x_new = (xValue,yValue,zValue)
        #make sure the added node doesn't intersect any obstacles
        for recto in obstacles:
            if lineHitsRect(x_near, x_new, recto):
                obstructed = 1
        #check the configuration of the robot if it fits
        newFrontx = x_new[0]+ (robot_length/2)*math.cos(x_new[2])
        newFronty = x_new[1]+ (robot_length/2)*math.sin(x_new[2])
        newFront = [newFrontx,newFronty]
        newBackx = x_new[0]+ (robot_length/2)*math.cos(x_new[2]+180)
        newBacky = x_new[1]+ (robot_length/2)*math.sin(x_new[2]+180)
        newBack = [newBackx,newBacky]
        for recto in obstacles:
            if lineHitsRect(newFront, newBack, recto):
                obstructed = 1
        #Add to graph
        if obstructed == 0:
            lengthv = len(G[nodes])
            G[nodes].append(lengthv)
            G[edges].append((v_near, pointToVertex(x_new)))
            canvas.polyline([x_near, x_new])
            #check goal
            if pointPointDistance(x_new, goalPos) <= SMALLSTEP*2:
                parentNode = lengthv
                lastIter = G[nodes][0]
                pathCount = 0
                while parentNode != lastIter:
                    lastIter = parentNode
                    parentNode = returnParent(parentNode)
                    pathCount += 1
                #canvas.events()
                print('path length: ', pathCount)
                break
        canvas.events()
