...
def genPoint():
    ...
    rand = None
    while rand is None:
        rand = (min(XMAX, max(0,random.gauss(tx,340))),
                min(YMAX, max(0,random.gauss(ty,340))))
...
def rrt_search(G, tx, ty):
    ...
    while found != 1:
        ...
        #If point is closer than step, add it to graph
        if pointPointDistance(x_rand, x_near) <= SMALLSTEP:
            xValue = x_rand[0]
            yValue = x_rand[1]
        #Otherwise get closer point of size step
        else:
        ...
        x_new = (xValue,yValue)
        #Make sure the edge isn't obstructed
        for recto in obstacles:
